...
class HopfieldNode:
	def __init__(self, networkIndex, nodeName, wordVector, nodeGraphType, activationTime, biologicalSimulation, w, sentenceIndex):
		...
		if(biologicalSimulation):
			HopfieldNodeBiologicalSimulationPropertiesInitialisation(self)
...
def biologicalSimulationNodePropertiesInitialisation(conceptNode):
	...
	if(vectoriseComputationCurrentDendriticInput):
		conceptNode.vectorisedBranchActivationLevelList, conceptNode.vectorisedBranchActivationTimeList, conceptNode.vectorisedBranchObjectList = createBatchDendriticTreeVectorised(batched=False)	#shape [numberOfHorizontalBranches, horizontalBranchWidth, numberOfBranchSequentialSegments]
